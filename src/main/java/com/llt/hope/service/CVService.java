...
public class CVService {
    ...
    public PageResponse<CVResponse> getAll(int page, int size) {
        ...
        List<CVResponse> cvResponses =
                cvForms.getContent().stream().map(cvMapper::toCVResponse).collect(Collectors.toList());
    ...
    public CVResponse get(Long id) {
        CVForm cvForm = cvRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.CV_NOT_FOUND));
        return cvMapper.toCVResponse(cvForm);
    }

    public CVResponse update(CVUpdateReq req) {
        CVForm cvForm =
                cvRepository.findById(req.getCvId()).orElseThrow(() -> new AppException(ErrorCode.CV_NOT_FOUND));
