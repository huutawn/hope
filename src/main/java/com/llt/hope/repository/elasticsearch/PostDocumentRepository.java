...
public interface PostDocumentRepository extends ElasticsearchRepository<PostDocument, String> {
    ...
    @Query(
            "{\"bool\": {\"must\": [{\"multi_match\": {\"query\": \"?0\", \"fields\": [\"title^3\", \"content^2\", \"userName\"]}}], \"filter\": [{\"term\": {\"isActive\": true}}]}}")
    ...
    Page<PostDocument> findByUserNameContainingIgnoreCaseAndIsActiveTrueAndIsPublishedTrueOrderByCreatedAtDesc(
    ...
    Page<PostDocument> findByTitleContainingIgnoreCaseAndIsActiveTrueAndIsPublishedTrueOrderByCreatedAtDesc(
            String title, Pageable pageable);

    // Search posts by content
    Page<PostDocument> findByContentContainingIgnoreCaseAndIsActiveTrueAndIsPublishedTrueOrderByCreatedAtDesc(
            String content, Pageable pageable);

    // Advanced search for posts with multiple filters
    @Query(
            "{\"bool\": {\"must\": [{\"multi_match\": {\"query\": \"?0\", \"fields\": [\"title^3\", \"content^2\", \"userName\"]}}], \"filter\": [{\"term\": {\"isActive\": true}}, {\"term\": {\"isPublished\": true}}]}}")
    Page<PostDocument> findByAdvancedSearch(
            String keyword, String type, Long userId, Boolean isPinned, Pageable pageable);
}
