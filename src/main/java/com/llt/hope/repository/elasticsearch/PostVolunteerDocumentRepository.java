...
public interface PostVolunteerDocumentRepository extends ElasticsearchRepository<PostVolunteerDocument, String> {
    ...
    @Query(
            "{\"bool\": {\"must\": [{\"multi_match\": {\"query\": \"?0\", \"fields\": [\"title^3\", \"content^2\", \"location^2\", \"userName\", \"bankName\"]}}], \"filter\": [{\"term\": {\"isActive\": true}}]}}")
    Page<PostVolunteerDocument> findByKeyword(String keyword, Pageable pageable);

    // Search volunteer posts by location
    Page<PostVolunteerDocument> findByLocationContainingIgnoreCaseAndIsActiveTrueOrderByCreateAtDesc(
    ...
    Page<PostVolunteerDocument> findByUserNameContainingIgnoreCaseAndIsActiveTrueOrderByCreateAtDesc(
    ...
    @Query(
            "{\"bool\": {\"must\": [{\"range\": {\"fund\": {\"gte\": ?0, \"lte\": ?1}}}], \"filter\": [{\"term\": {\"isActive\": true}}]}}")
    ...
    @Query(
            "{\"bool\": {\"must\": [{\"range\": {\"totalAmount\": {\"gte\": ?0, \"lte\": ?1}}}], \"filter\": [{\"term\": {\"isActive\": true}}]}}")
    ...
    Page<PostVolunteerDocument> findByTitleContainingIgnoreCaseAndIsActiveTrueOrderByCreateAtDesc(
            String title, Pageable pageable);

    // Search volunteer posts by content
    Page<PostVolunteerDocument> findByContentContainingIgnoreCaseAndIsActiveTrueOrderByCreateAtDesc(
            String content, Pageable pageable);

    // Search volunteer posts by bank name
    Page<PostVolunteerDocument> findByBankNameContainingIgnoreCaseAndIsActiveTrueOrderByCreateAtDesc(
            String bankName, Pageable pageable);

    // Advanced search for volunteer posts with multiple filters
    @Query(
            "{\"bool\": {\"must\": [{\"multi_match\": {\"query\": \"?0\", \"fields\": [\"title^3\", \"content^2\", \"location^2\", \"userName\", \"bankName\"]}}], \"filter\": [{\"term\": {\"isActive\": true}}]}}")
    Page<PostVolunteerDocument> findByAdvancedSearch(
            String keyword,
            String location,
            String status,
            Long userId,
