...
public interface MessageRepository extends JpaRepository<Message,Long> {
    ...
    List<Message> findMessagesBetweenUsers(@Param("user1Email") String user1Email, @Param("user2Email") String user2Email);

    @Query("SELECT m FROM Message m WHERE m.sender.email = :sender AND m.receiver.email = :receiver AND m.isRead = false")
